...
query = Message(role_name="tool_react", role_type="human", 
                input_query="我有一份时序数据，[0.857, 2.345, 1.234, 4.567, 3.456, 9.876, 5.678, 7.890, 6.789, 8.901, 10.987, 12.345, 11.234, 14.567, 13.456, 19.876, 15.678, 17.890, 16.789, \
...
query = Message(role_name="tool_react", role_type="human", 
                input_query="帮我确认下127.0.0.1这个服务器的在10点是否存在异常，请帮我判断一下", tools=tools)

query = Message(role_name="code_react", role_type="human", 
                input_query="帮我确认当前目录下有哪些文件", tools=tools)

# "给我一份冒泡排序的代码"
query = Message(role_name="user", role_type="human", 
                input_query="对employee_data.csv进行数据分析", tools=tools)
...
# query = Message(role_name="user", role_type="human", 
#                 input_query="确认本地是否存在book_data.csv，并查看它有哪些列和数据类型，分析这份数据的内容，根据这个数据预测未来走势", tools=tools)
# query = Message(role_name="user", role_type="human", 
#                 input_query="确认本地是否存在employee_data.csv，并查看它有哪些列和数据类型;然后画柱状图", tools=tools)
# chain = BaseChain(chain_configs["executorChain"], agents, do_code_exec=False, chat_turn=1, do_checker=False)

# output_message, local_memory = chain.step(query)
